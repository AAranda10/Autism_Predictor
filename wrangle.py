...
def train_validate_test(df):
    ...
    X_train = train.drop(columns= ['case_no', 'has_asd'])
    X_validate = validate.drop(columns= ['case_no', 'has_asd'])
    X_test = test.drop(columns= ['case_no', 'has_asd'])

    y_train = train[['has_asd']]
    y_validate = validate[['has_asd']]
    y_test = test[['has_asd']]
...
def min_max_scale(X_train, X_validate, X_test):
    ...
    X_train = X_train.drop(columns= ['ethnicity', 'whocompletedthetest', 'has_jaundice', 'has_fam_history',
       'is_white', 'is_hispanic', 'is_black', 'is_asian', 'is_middle_eastern',
       'is_other', 'survey_score'])
    X_validate = X_validate.drop(columns= ['ethnicity', 'whocompletedthetest', 'has_jaundice', 'has_fam_history',
       'is_white', 'is_hispanic', 'is_black', 'is_asian', 'is_middle_eastern',
       'is_other', 'survey_score'])
    X_test = X_test.drop(columns= ['ethnicity', 'whocompletedthetest', 'has_jaundice', 'has_fam_history',
       'is_white', 'is_hispanic', 'is_black', 'is_asian', 'is_middle_eastern',
       'is_other', 'survey_score'])
